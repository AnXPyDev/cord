...
do
  ...
  local _base_0 = {
    ...
    create_stylizers = function(self)
      self.stylizers.size = function(sole)
      ...
      self.stylizers.padding = function(sole)
        local padding = self.current_style:get("padding")
        padding:apply(self.containers.padding)
        self:emit_signal("stylized", "padding")
        if sole then
          return self.widget:emit_signal("widget::redraw_needed")
        end
      end
      self.stylizers.margin = function(sole)
        local margin = self.current_style:get("margin")
        margin:apply(self.containers.margin)
        self:emit_signal("stylized", "margin")
        if sole then
          return self.widget:emit_signal("widget::redraw_needed")
        end
      end
      self.stylizers.background = function(sole)
        ...
        self:emit_signal("stylized", "background")
        if sole then
          return self.widget:emit_signal("widget::redraw_needed")
        end
      end
      self.stylizers.overlay = function(sole)
        ...
        self:emit_signal("stylized", "overlay")
        if sole then
          return self.widget:emit_signal("widget::redraw_needed")
        end
      end
      self.stylizers.background_shape = function(sole)
        self.containers.background.shape = self.current_style:get("background_shape")
        self:emit_signal("stylized", "background_shape")
        if sole then
          return self.widget:emit_signal("widget::redraw_needed")
        end
      end
      self.stylizers.overlay_shape = function(sole)
        self.containers.background.shape = self.current_style:get("background_shape")
        self:emit_signal("stylized", "overlay_shape")
        if sole then
          return self.widget:emit_signal("widget::redraw_needed")
        end
      end
  ...
  _class_0 = setmetatable({
    __init = function(self, category, label, stylesheet, children)
      ...
      self.label = label
      self.identification = "node " .. tostring(self.category) .. " " .. tostring(self.label) .. " " .. tostring(self.unique_id)
