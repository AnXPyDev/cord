...
do
  ...
  local _base_0 = {
    add_style = function(self, identification, parents)
      ...
      local category = identification[1]
      local label = identification[2]
      ...
      local style = self:get_style(category, label)
      for k, v in pairs(parents) do
        style:add_parent(self:get_style(v))
      end
      return style
    end,
    get_style = function(self, identification)
      if identification == nil then
        identification = { }
      end
      local category = identification[1]
      local label = identification[2]
      if label and label ~= "__empty_label__" and self.by_label[label] then
        return self.by_label[label]
      elseif category and category ~= "__empty_category__" and self.by_category[category] then
  ...
  _class_0 = setmetatable({
    __init = function(self)
      ...
      table.insert(self.__name, "cord.wim.stylsheet")
      self.by_category = { }
      self.by_label = { }
    end,
    __base = _base_0,
